...
-- (REQ-3) — rank_no_orgao/perc_sobre_total_orgao particionados por ano.
-- ─────────────────────────────────────────────────────────────────────────────
...
with base as (
    ...
    -- deste model já é agregado (SUM por órgão/fornecedor/ano), filtro
    ...
    select
        *,
        sum(vl_total_fornecedor_orgao) over (
            partition by cod_unidade_gestora, ano_assinatura
        )                                                as vl_total_orgao,
