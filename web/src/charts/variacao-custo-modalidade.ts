...
export async function renderVariacaoCustoModalidade(containerId: string, tableId: string): Promise<void> {
  ...
  const ordenadas = [...linhas].sort((a, b) => Number(b.perc_variacao_media) - Number(a.perc_variacao_media));
  ...
  chart.setOption({
    ...
    series: [
      {
        type: "bar",
        data: ordenadas.map((l) => Number(l.perc_variacao_media)).reverse(),
      },
    ],
  });
