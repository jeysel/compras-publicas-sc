...
| Validação antes do `dbt run` | Checagem mínima de schema: colunas esperadas presentes (subconjunto, case-insensitive — ver [[017-validacao-schema-tolerante]]), contagem de linhas não-zero, não é arquivo corrompido (tipo o `contratos-2022.csv` com aspas malformadas da spec 006) | Se falhar: o Job termina com erro, **não sobrescreve** o dado bom já processado, e o `ETag` salvo **não é atualizado** (tenta de novo no próximo ciclo, não fica preso presumindo que já processou). Lógica de comparação exata original revisada pela spec 017 (achado da spec 016: mudança real do portal foi aditiva, comparação exata era mais rígida do que pedido aqui). |
...
- [[017-validacao-schema-tolerante]] (revisão da lógica de validação de schema)
