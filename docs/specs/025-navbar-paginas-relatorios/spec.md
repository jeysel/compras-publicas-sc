...
- REQ-9a: A página do relatório de variação de prazo DEVE exibir aviso explícito de que `dias_originais`/`dias_atuais` estão preenchidos em só ~7% dos contratos na fonte (achado 4a da Investigação) — a tabela/gráfico não deve ser apresentado sem esse contexto, sob risco de o usuário interpretar "sem dado" como "sem aditivo de prazo".
